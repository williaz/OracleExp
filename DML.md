...
- V_$, GV_$: dynamic performace view/global ..; V$, GV$
  1 V$DATABASE
  2 V$INSTANCE
  3 V$PARAMETER
  4 V$SESSION
...
COMMENT ON TABLE tab IS comment_str;
COMMENT ON COLUMN tab.col IS c; -- c='' to drop it
...
COMMENT ON TABLE emp IS 'copy of HR.EMPLOYEES';
SELECT * FROM USER_TAB_COMMENTS;
COMMENT ON COLUMN emp.email IS 'email addess';
SELECT * FROM USER_COL_COMMENTS;

SELECT * FROM DICTIONARY;
```
- USER_CATALOG/CAT: a summary list od TABLE, VIEW, SYNONYM, and SEQUENCE
```sql
SELECT * FROM USER_CATALOG;
SELECT TABLE_TYPE, COUNT(*) FROM CAT GROUP BY TABLE_TYPE;
...
- SESSION_PRIVS
### USER_CONSTRAINTS
- CONSTRAINT_TYPE: P, R(FK), U, C(CHECK/NOT NULL)
- DELETE_RULE: ON DELETE CASCADE/ ON DELETE SET NULL
