...
- Use 12c enhancements to the DEFAULT clause, invisible columns, virtual columns and identity columns in table creation/alteration
  - Sequence can be used for Column DEFAULT
  - Invisible column
  - Virtual column
  - identity column
    - only one per table
    - must be a numeric datatype
    - cannot also use DEFAULT
    - implicit NOT NULL and NOT DEFERRABLE
```sql
CREATE SEQUENCE temp_seq START WITH 1 INCREMENT BY 1;

CREATE TABLE worker (
work_id number DEFAULT temp_seq.nextval,
name varchar2(20)
);

insert into worker (name) VALUES ('will');
insert into worker (name) VALUES ('bill');

select * from worker;

alter table worker add ( age number invisible);
insert into worker (name, age) VALUES ('kevin', 26);

select work_id, name, age from worker;

alter table worker add (salary as (age*200));

drop table counter;
create table counter (
count_id number generated by default as identity primary key,
count_peo varchar2(20)
);

insert into counter values (2, 'will');
insert into counter (count_peo) values ('bill');
insert into counter (count_peo) values ('qill'); -- 2 error, 3
